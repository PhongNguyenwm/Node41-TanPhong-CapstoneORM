...
export const middleware = {
  ...
  protect: async (req, res, next) => {
    try {
      ...
      if (!accessToken || !accessToken.startsWith("Bearer "))
        return helper.responses(res, 400, "Not enough permissions");
  ...
  checkLoginRequest: (req, res, next) => {
    if (!req.body.userName)
      return helper.responses(res, 400, "Invalid username");

    if (!req.body.password)
      return helper.responses(res, 400, "Invalid password");
  ...
  checkRegisterRequest: (req, res, next) => {
    ...
    if (!req.body.phoneNumber)
      return helper.responses(res, 400, "Invalid phone number");
    if (!req.body.fullName)
      return helper.responses(res, 400, "Invalid fullname");
  ...
  checkCreateImageRequest: (req, res, next) => {
    ...
    if (req.file.size > 1000000)
      return helper.responses(res, 400, "File size cannot exceed 1mb");
    if (!req.body.imageName)
      return helper.responses(res, 400, "Invalid imageName");
    next();
  },
